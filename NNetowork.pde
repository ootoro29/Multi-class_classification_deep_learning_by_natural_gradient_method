...
class NhideLayor extends Nlayor {
  NhideLayor(int x, int NSIZE) {
    super(x, NSIZE);
  }
  Matrix f(Matrix x) {
...
class NoutLayor extends Nlayor {
  NoutLayor(int x, int NSIZE) {
    super(x, NSIZE);
  }
  Matrix f(Matrix x) {
    Matrix ans = new Matrix(x);
...
class NNeural {
  ...
  int NSIZE = 0;
  double alpha = 1;
  ArrayList<Nlayor>net = new ArrayList();
  Matrix ans[], in[], out[];
  NNeural(int []size, int NSIZE,double alpha) {
    this.NSIZE = NSIZE;
    this.alpha = alpha;
    N = size.length;
    ...
    for (int i = 0; i < N-1; i++) {
      net.add(new NhideLayor(size[i], NSIZE));
      net.get(net.size()-1).alpha = this.alpha;
    }
    net.add(new NoutLayor(size[N-1], NSIZE));
    net.get(0).state = -1;
    net.get(N-1).state = 1;
    for (int i = 1; i< N; i++) {
